 ...
 */
export function* SplitAndUnescape(s: string, delim: string): Generator<string> {
    if (delim.length != 1) {
 ...
 * @param {string} delim - The delimiter used to split, it should be only a single char
 */
export function EscapeAndJoin(parts: string[], delim: string): string {
    if (delim.length != 1) {
        throw new Error(`invalid delim: '${delim}'`);
    }
...
export class AppendMergeSegment<T> {
    ...
    private deserialize: (s: string) => [string, T | null];
    private serialize: (k: string, v: T | null) => string;
    private lastSerialized: string;
    private current?: Map<string, T>;
    private delta: Map<string, T | null>;
    ...
    *entries(): Generator<[string, T]> {
    ...
    write(compactLimit: number): string | null {
        ...
        if (compactLimit == 0 || this.delta.size >= current.size * 0.7) {
            ...
            const serializedDelta = EscapeAndJoin(
                [...this.delta.entries()].map(([k, v]) => this.serialize(k, v)),
                this.delim,
            );
        ...
        return (this.lastSerialized = EscapeAndJoin(
            [...current.entries()].map(([k, v]) => this.serialize(k, v)),
            this.delim,
        ));
    }

    private initCurrent(): Map<string, T> {
        if (this.current) {
            return this.current;
        }
