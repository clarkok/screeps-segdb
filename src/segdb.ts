...
import { AppendMergeSegment, EscapeAndJoin, SplitAndUnescape } from './append_merge_segment';

interface SegDBMemory {
    partitions: {
        // [low, high), where null in the high represents MAX
        low: string;
        high: string | null;
        seg: number;
    }[];
    allocateBegin: number;
    allocateLimit: number;
    writing: number;
    freedSegments: number[];
}
...
function getDefaultSegDBConfig(): SegDBConfig {
    return {
...
export class SegDB {
    private config: SegDBConfig;
    private segments: Map<number, AppendMergeSegment<string>>;
    private writing: AppendMergeSegment<string> | null;
    private estimatedWritingSize: number;
    private getRequests: {
        key: string;
        cb: (v: string | undefined, k: string) => void;
    }[];
    private listRequests: {
        key: string;
        cb: (key: string | null, value: string | null) => SegDBListControl | void;
        proc: Generator<void> | null;
    }[];

    private checkpointing: string | null;
    private splitQueue: number[];
    private merging: string | null;
    private compacting: string | null;
    ...
    constructor(conf?: Partial<SegDBConfig>) {
        this.config = Object.assign(getDefaultSegDBConfig(), conf ?? {});
    ...
    private writeInternal(k: string, v: string): boolean {
        if (!this.writing) {
            return false;
        }
    ...
    list(start: string, cb: (key: string | null, value: string | null) => SegDBListControl | void) {
        const req: SegDB['listRequests'][0] = {
            key: start,
            cb,
            proc: null,
        };
    ...
    private *listProc(req: SegDB['listRequests'][0]): Generator<void> {
        while (true) {
            ...
            while (wIdx != writingKeys.length || sIdx != segKeys.length) {
                ...
                ctrl = req.cb(req.key, value) ?? SegDBListControl.Continue;
    ...
    private checkpoint(writeQuotaForPartitions: number, cpuLimit: number): number {
    ...
    private split(writeQuotaForPartitions: number, cpuLimit: number) {
    ...
    private splitPartition(seg: number, newSeg: number, leftRatio: number) {
        const loadedSegment = this.segments.get(seg);
        if (!loadedSegment) {
            return;
        }
    ...
    private merge(writeQuotaForPartitions: number, cpuLimit: number) {
        while (Game.cpu.getUsed() <= cpuLimit && this.merging !== null && writeQuotaForPartitions) {
            ...
            if (
                loadedSegment.lastSerializedLength + nextLoadedSegment.lastSerializedLength <=
    ...
    private mergePartitions(
        leftPart: SegDBMemory['partitions'][0],
        rightPart: SegDBMemory['partitions'][0],
        leftSegment: AppendMergeSegment<string>,
        rightSegment: AppendMergeSegment<string>,
    ) {
        ...
        Memory.segdb.partitions.splice(
            Memory.segdb.partitions.findIndex(p => p.seg == rightPart.seg),
            1,
        );
        this.segments.delete(rightPart.seg);
    }

    private compact(writeQuotaForPartitions: number, cpuLimit: number) {
    ...
    private rebalancePartitions(
        ...
        let newSplitKey: string | null = null;
    ...
    private static getPartitionForKey(k: string): Memory['segdb']['partitions'][0] {
        return Memory.segdb.partitions.find(p => p.low <= k && (p.high === null || p.high > k))!;
    }

    private static getPartitionSegForKey(k: string): number {
        return SegDB.getPartitionForKey(k).seg;
    }

    private static deserializeString(s: string): [string, string | null] {
        const split = [...SplitAndUnescape(s, '=')];
        if (split[1] == '') {
            return [split[0], null];
        }
    ...
    private static serializeString(k: string, v: string | null): string {
        if (v == null) {
            v = '';
        }
        return EscapeAndJoin([k, v], '=');
    }

    private static createAppendMergeSegment(lastSerialized: string): AppendMergeSegment<string> {
        return new AppendMergeSegment<string>('|', lastSerialized, this.deserializeString, this.serializeString);
    }
}
